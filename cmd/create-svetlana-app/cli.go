...
func parseArguments(arguments ...string) Command {
	...
	// 		"- create-svetlana-app --template=javascript app-name\n\n" +
	// 		"Or\n\n" +
	// 		"- create-svetlana-app --template=typescript app-name")
	// 	os.Exit(2)
	// }
	if len(flagset.Args()) == 0 {
		loggers.Error("It looks like you’re trying to run `create-svetlana-app` in the current directory. " +
			"In that case, use `.` explicitly.\n\n" +
			"- create-svetlana-app .\n\n" +
			"Or\n\n" +
			"- create-svetlana-app app-name")
		os.Exit(2)
	}
	cmd.Directory = "."
	if len(flagset.Args()) > 0 {
